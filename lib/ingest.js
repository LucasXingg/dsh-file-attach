...
/** Filename extensions that map onto {@link IMAGE_MEDIA_TYPES}. */
const IMAGE_EXTENSIONS = new Set(['png', 'jpg', 'jpeg', 'webp', 'gif'])

/** Basename extension, lowercased; empty when the name has no extension. */
...
/**
 * True when declared MIME or filename is a raster image this plugin (or DSH's
 * native vision path) can handle.
 */
export function isRasterImage({ mime, name } = {}) {
  if (IMAGE_MEDIA_TYPES.includes(mime)) return true
  return IMAGE_EXTENSIONS.has(extensionOfName(name))
}

/**
 * Canonical raster MIME for a file, from declared type or extension.
 * @returns a member of {@link IMAGE_MEDIA_TYPES}, or undefined.
 */
export function rasterMediaType({ mime, name } = {}) {
  if (IMAGE_MEDIA_TYPES.includes(mime)) return mime
  switch (extensionOfName(name)) {
 ...
 * @param files - iterable of `{ name, type? | mime? }`.
 */
export function partitionIntake(files) {
  const images = []
  const others = []
  for (const file of files ?? []) {
    const mime = file?.type || file?.mime
    if (isRasterImage({ mime, name: file?.name })) images.push(file)
 ...
 * Missing `inputModalities` is unknown, not visual — callers fall back to the
 * plugin extract path.
 */
export function modelSupportsVisual(info) {
...
/**
 * Hidden appendix block for one file. The header sits inside the fence so the
 * conversation UI can strip the whole section; the composer filename stays
 * as the only user-visible reference.
 */
export function extractSection(meta) {
  const extractText = meta?.extract?.text
  if (typeof extractText !== 'string' || meta?.name === undefined) return ''
...
export function modelForm(meta) {
  ...
  const section = extractSection(meta)
...
/**
 * Drop extract fences so a display projection shows the header without the
 * prompt body. Markers are matched as plain substrings — a renderer may
 * collapse the newlines around them — and a start marker that never gets its
 * end marker drops everything after it, so no fence body can survive.
 *
 * Mirrors `stripExtractForDisplay` in src/client-core.js; keep both in sync.
 */
export function stripExtractForDisplay(text) {
  ...
  let cursor = 0
  for (;;) {
    const start = input.indexOf(EXTRACT_FENCE_START, cursor)
    if (start === -1) return out + input.slice(cursor)
    out += input.slice(cursor, fenceCutFrom(input, start))
    const body = start + EXTRACT_FENCE_START.length
    const end = input.indexOf(EXTRACT_FENCE_END, body)
    const next = input.indexOf(EXTRACT_FENCE_START, body)
    // Fences never nest: a second start marker first means this one lost its end.
    if (end === -1 || (next !== -1 && next < end)) return out
    cursor = end + EXTRACT_FENCE_END.length
  }
}

const ATTACH_ID_RE = /\bid=([0-9a-f]{12})\b|\/attach\s+([0-9a-f]{12})\b/g

/**
 * First-appearance order of 12-character attach ids in composer/model text.
 * Matches the blue filename header (`id=…`) and a recovered `/attach` line.
 */
export function attachIdsInText(text) {
  const ids = []
  const seen = new Set()
  ATTACH_ID_RE.lastIndex = 0
  let match
  while ((match = ATTACH_ID_RE.exec(String(text))) !== null) {
...
/**
 * Keep the user prompt (and any inline filenames) and put every extract
 * behind it, in first-reference order. Existing extract fences are moved,
 * not duplicated.
 */
export function appendExtractsBehindPrompt(text, metas) {
  const stripped = stripExtractForDisplay(text)
  const wanted = attachIdsInText(stripped)
  if (wanted.length === 0) return stripped
  const byId = new Map()
  for (const meta of metas ?? []) {
    if (meta != null && typeof meta.id === 'string') byId.set(meta.id, meta)
  }
  const parts = []
  for (const id of wanted) {
    const section = extractSection(byId.get(id))
