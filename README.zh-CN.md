...
- 与输入框同宽的附件栏，显示由插件上传的文件、大小，以及跟随输入框里蓝色
  文件名的移除按钮。
...
提取完成后，插件最多进行两次带草稿版本保护的尝试，把蓝色文件名插到草稿末尾
（就是输入框里可用退格键删掉的那个文件名）。如果插入失败，客户端会显示错误；
已完成文件仍保留在文件库中，附件栏会留下一条可关闭、但输入框里没有蓝色文件
名的记录。

已完成附件的 **×** 会从输入框里删掉对应的蓝色文件名；原文件仍留在文件库。
附件栏中的已完成条目跟随输入框：用退格键删掉那个文件名，或发送消息（草稿被
清空）后，该文件会从输入框上方消失。进行中的上传会继续显示，直到完成或失败。
...
输入框里的蓝色文件名只是给模型的短引用，可以放在提示词的任何位置，提取内容
不会在那里展开。发送时，宿主会把这些文件的提取结果全部接到**用户提示词后面**：
...
原文件和 `extract.json` 仍保存在文件库。宿主仍会从文件库把提取内容接到提示
词后面，工具也能通过 ID 找到原文件。
...
每分钟扫描一次。已完成上传不会自动清理：从输入框或附件栏移除引用不会删除原
文件。插件卸载时，浏览器会中止活动上传，宿主会清理仍打开的上传会话。
